...
export default function Insights({ onNavigate }: InsightsProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Seed with demo data for charts and counters
      const days = 12;
      const now = new Date();
      const demo: MoodData[] = Array.from({ length: days }).map((_, i) => {
        const d = new Date(now);
        d.setDate(d.getDate() - i);
        return {
          date: d.toISOString().split('T')[0],
          mood_level: Math.max(1, Math.min(5, Math.round(3 + Math.sin(i / 2) * 1.5))),
          mood_type: ['sad','anxious','neutral','happy','calm'][Math.max(0, Math.min(4, Math.round(2 + Math.sin(i)) ))]
        };
      }).reverse();
      setMoodData(demo);
      setStats({ totalMoodLogs: demo.length, totalJournals: 5, totalGratitude: 4, avgMood: Number((demo.reduce((s,m)=>s+m.mood_level,0)/demo.length).toFixed(1)) });
      setStreak({ current: 3, longest: 7 });
      setAchievements([{ badge_name: 'First Step', earned_at: new Date().toISOString() }]);
      setLoading(false);
    }
  }, [user]);
